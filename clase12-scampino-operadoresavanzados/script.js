...
localStorage.getItem("productos filtrados") && mostrarEnDom()
...
function mostrarEnDom(){
    ...
    precioConsultado1.length >= 1 ? cardsFiltro.innerHTML = `<h2 id="tituloBusqueda">Se muestran productos hasta los $${tituloStorage}</h2>` : cardsFiltro.innerHTML = `<h2 id="tituloBusqueda">No hay productos en ese precio</h2>`
...
// Uso de desestructuración en parámetros de objetos
const desestructurar = ( {precio} ) => {
    console.log(precio * 1.21)
}
 
guiatarrasEnArray.forEach((sumandoIva) =>{
    desestructurar(sumandoIva);
})